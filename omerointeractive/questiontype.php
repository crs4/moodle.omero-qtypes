 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later // FIXME: check the licence
 */
class qtype_omerointeractive extends qtype_multichoice
{
...
//    }
    ...
    protected function make_question_instance($questiondata)
    {
        ...
        if ($questiondata->options->single) {
            $class = 'qtype_omerointeractive_single_question';
        } else {
    ...
    {
        global $DB, $OUTPUT;
        $question->options = $DB->get_record('qtype_omeinteractive_options',
    ...
    public function save_question_options($question)
    {
        ...
        if ($answercount < 2) { // Check there are at lest 2 answers for multiple choice.
            $result->notice = get_string('notenoughanswers', 'qtype_interactive', '2');
            return $result;
        }
        ...
        $options = $DB->get_record('qtype_omeinteractive_options', array('questionid' => $question->id));
        if (!$options) {
            $options = new stdClass();
            $options->questionid = $question->id;
            $options->answertype = qtype_omerointeractive::PLAIN_ANSWERS;
            $options->omeroimageurl = '';
            $options->correctfeedback = '';
            $options->partiallycorrectfeedback = '';
            $options->incorrectfeedback = '';
            $options->visiblerois = '';
            $options->id = $DB->insert_record('qtype_omeinteractive_options', $options);
        }
        ...
        $DB->update_record('qtype_omeinteractive_options', $options);
        ...
        if ($options->single) {
            if ($maxfraction != 1) {
                $result->noticeyesno = get_string('fractionsnomax', 'qtype_interactive',
                    $maxfraction * 100);
                return $result;
            }
        } else {
            ...
            if ($totalfraction != 1) {
                $result->noticeyesno = get_string('fractionsaddwrong', 'qtype_interactive',
                    $totalfraction * 100);
                return $result;
            }
        }
...
//    }
    ...
    protected function initialise_question_instance(question_definition $question, $questiondata)
    {
        ...
        if(!empty($questiondata->options->answertype)) {
            ...
            $question->answertype = qtype_omerointeractive::PLAIN_ANSWERS;
        }
    }
    ...
    {
        global $DB;
        $DB->delete_records('qtype_omeinteractive_options', array('questionid' => $questionid));
    ...
    public static function get_question_types(){
        return array(qtype_omerointeractive::PLAIN_ANSWERS, qtype_omerointeractive::ROI_BASED_ANSWERS);
    }
}
